...
<div class="container-fluid">
  ...
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800"> All Auctions Items</h1>
    
    <a href="{{ route('addnew_auction_item') }}" class="btn-success btn btn-sm shadow-lg border-0"> Add New Record</a>
  </div> 
    ...
    <div class="card-body">
       ...
       <div class="row input-daterange">
         ...
         <div class="col-md-2">
          ...
          <select name="parient_auction" class="form-control parient_auction" id="parient_auction">
            @isset ($auctions)
        ...
        <div class="col-md-1">
          <button type="button" name="filter" id="filter" class="btn btn-primary btn-block btn-sm shadow-lg border-0">Filter</button>
        </div>
        <div class="col-md-1">
          <button type="button" name="refresh" id="refresh" class="btn btn-danger btn-block btn-sm shadow-lg border-0"><i class="fas fa-sync-alt"></i></button>
        </div>
...
if (willDelete) {
$.ajax({
url:"{{ route('delete_Auctions_item_ajax') }}",
type:"POST",
dataType:"json",
...
function load_data(from_date='', to_date='',parient_auction='',status='',typep=''){
$('#dataTableAuction').DataTable({
processing: true,
serverSide: true,
responsive: true,
ajax:{
url: "{{ route('getall_Auctions_item_ajax') }}",
data:{from_date:from_date, to_date:to_date,parient_auction:parient_auction,status:status,typep:typep}
},
columns:[
{
data: 'featured_img',
...
var JSONString = full.gallery;
var nameArr = JSONString.split(',');
  
return "<img src={{ URL::to('/') }}/img/" + nameArr[0] + " class='rounded-circle mr-1' width='60' height='60'/>";
},
orderable: false
},
{
...
if(!isEmpty(from_date)  && !isEmpty(to_date) && !isEmpty(parient_auction) && !isEmpty(status) && !isEmpty(typep))
{
$('#dataTableAuction').DataTable().destroy();
load_data(from_date, to_date,parient_auction,status,typep);
}
else
{
toastr.options.closeButton = true;
